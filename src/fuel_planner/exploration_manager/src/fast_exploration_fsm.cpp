...
namespace fast_planner
{
  void FastExplorationFSM::init(ros::NodeHandle &nh)
  {
    ...
    exploration_mode_pub = nh.advertise<std_msgs::String>("/exploration_mode", 10);
  }
  ...
  void FastExplorationFSM::FSMCallback(const ros::TimerEvent &e)
  {
    ...
    case INIT:
    {
      fast_exploration_mode.data = "INIT"; // Set the string data
      exploration_mode_pub.publish(fast_exploration_mode);
    ...
    case WAIT_TRIGGER:
    {
      fast_exploration_mode.data = "WAIT_TRIGGER"; // Set the string data
      exploration_mode_pub.publish(fast_exploration_mode);
    ...
    case FINISH:
    {
       fast_exploration_mode.data = "FINISH"; // Set the string data
      exploration_mode_pub.publish(fast_exploration_mode);
    ...
    case PLAN_TRAJ:
    {
      fast_exploration_mode.data = "PLAN_TRAJ"; // Set the string data
      exploration_mode_pub.publish(fast_exploration_mode);
    ...
    case PUB_TRAJ:
    {
      fast_exploration_mode.data = "PUB_TRAJ"; // Set the string data
      exploration_mode_pub.publish(fast_exploration_mode);
    ...
    case EXEC_TRAJ:
    {
      fast_exploration_mode.data = "EXEC_TRAJ"; // Set the string data
      exploration_mode_pub.publish(fast_exploration_mode);
      LocalTrajData *info = &planner_manager_->local_data_;
