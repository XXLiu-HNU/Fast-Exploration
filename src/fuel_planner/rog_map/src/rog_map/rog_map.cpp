...
void ROGMap::cloudCallback(const sensor_msgs::PointCloud2ConstPtr& cloud_msg) {
    ...
    // Check if the point cloud has the required fields
    bool has_intensity = false, has_normal = false, has_curvature = false;
    for (const auto& field : cloud_msg->fields) {
        if (field.name == "intensity") has_intensity = true;
        if (field.name == "normal_x") has_normal = true;
        if (field.name == "curvature") has_curvature = true;
    }
    
    // Convert based on available fields
    if (has_intensity && has_normal && has_curvature) {
        // Full PointXYZINormal format
        pcl::fromROSMsg(*cloud_msg, temp_pc);
    } else {
        // Convert from simpler formats and fill missing fields
        if (has_intensity) {
            // PointXYZI -> PointXYZINormal
            pcl::PointCloud<pcl::PointXYZI> cloud_xyzi;
            pcl::fromROSMsg(*cloud_msg, cloud_xyzi);
            temp_pc.resize(cloud_xyzi.size());
            for (size_t i = 0; i < cloud_xyzi.size(); ++i) {
                temp_pc[i].x = cloud_xyzi[i].x;
                temp_pc[i].y = cloud_xyzi[i].y;
                temp_pc[i].z = cloud_xyzi[i].z;
                temp_pc[i].intensity = cloud_xyzi[i].intensity;
                temp_pc[i].normal_x = 0.0;
                temp_pc[i].normal_y = 0.0;
                temp_pc[i].normal_z = 0.0;
                temp_pc[i].curvature = 0.0;
            }
        } else {
            // PointXYZ -> PointXYZINormal
            pcl::PointCloud<pcl::PointXYZ> cloud_xyz;
            pcl::fromROSMsg(*cloud_msg, cloud_xyz);
            temp_pc.resize(cloud_xyz.size());
            for (size_t i = 0; i < cloud_xyz.size(); ++i) {
                temp_pc[i].x = cloud_xyz[i].x;
                temp_pc[i].y = cloud_xyz[i].y;
                temp_pc[i].z = cloud_xyz[i].z;
                temp_pc[i].intensity = 0.0;
                temp_pc[i].normal_x = 0.0;
                temp_pc[i].normal_y = 0.0;
                temp_pc[i].normal_z = 0.0;
                temp_pc[i].curvature = 0.0;
