...
namespace fast_planner
{
                                 ...
                                 const nav_msgs::OdometryConstPtr &odom)
  {
    ...
    Eigen::Matrix3d body_r = camera_q_.toRotationMatrix();
    pcl::PointCloud<pcl::PointXYZ> cloud_filtered;
    const double min_dist_threshold = 0.2; // Filter points within 0.2m to avoid false detections from mounting
    
    for (int i = 0; i < num; ++i)
    {
      Eigen::Vector3d pt_body(cloud.points[i].x, cloud.points[i].y, cloud.points[i].z);
      Eigen::Vector3d pt_world = body_r * pt_body + camera_pos_;
      
      // Filter out points within 0.2m from camera position
      Eigen::Vector3d dist_vec = pt_world - camera_pos_;
    ...
    int num_filtered = cloud_filtered.points.size();
    map_->inputPointCloud(cloud_filtered, num_filtered, camera_pos_);
  ...
  void MapROS::publishMapAll()
  {
    ...
    ofstream file("/home/xingxun/Fast-Exploration/src/fuel_planner/exploration_manager/resource/"
                  "curve1.txt",
                  ios::app);
